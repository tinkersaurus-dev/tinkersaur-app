/**
 * Entity Relationship Diagram Toolset Popover Component
 *
 * Displays a popover with ER diagram tools when the user right-clicks on the canvas.
 * Tools are organized by type with icon buttons.
 */

import { entityRelationshipToolGroups, type Tool } from '@/features/diagram-rendering/entity-relationship/tools';
import type { DrawingConnector } from '../../../lib/hooks/useInteractionState';
import { ContextMenuWrapper } from '../ContextMenuWrapper';

interface EntityRelationshipToolsetPopoverProps {
...
export function EntityRelationshipToolsetPopover({
  x,
  y,
  canvasX,
  canvasY,
  ...
  onToolSelect,
  drawingConnector,
}: EntityRelationshipToolsetPopoverProps) {
  ...
  return (
    <ContextMenuWrapper
      menuId="entity-relationship-toolset-popover"
      isOpen={isOpen}
      x={x}
      y={y}
      onClose={onClose}
    ...
    >
      ...
      <div className="flex flex-col gap-0">
        {entityRelationshipToolGroups.map((group, groupIndex) => (
          <div key={group.type}>
            {/* Tool row */}
            ...
            {groupIndex < entityRelationshipToolGroups.length - 1 && (
