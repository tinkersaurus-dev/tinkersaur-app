/**
 * Personas sidebar component for user goal details
 * Displays linked personas with link/unlink functionality
 */

import { useState, useMemo } from 'react';
import { FiUser } from 'react-icons/fi';
import { Button, Card, CardStack, Modal } from '@/shared/ui';
import type { UserGoal } from '../model/types';
import type { Persona } from '@/entities/persona';
import { usePersonasQuery, PersonaCard } from '@/entities/persona';
import { useUpdateUserGoal } from '../api/mutations';

export interface UserGoalPersonasSidebarProps {
  userGoal: UserGoal;
  teamId: string | undefined;
}

export function UserGoalPersonasSidebar({ userGoal, teamId }: UserGoalPersonasSidebarProps) {
  const [isLinkModalOpen, setIsLinkModalOpen] = useState(false);

  const { data: allPersonas = [] } = usePersonasQuery(teamId);
  const updateUserGoal = useUpdateUserGoal();

  const linkedPersonaIds = useMemo(() => new Set(userGoal.personaIds), [userGoal.personaIds]);

  const linkedPersonas = useMemo(() => {
    return allPersonas.filter((p: Persona) => linkedPersonaIds.has(p.id));
  }, [allPersonas, linkedPersonaIds]);

  const availablePersonas = useMemo(() => {
    return allPersonas.filter((p: Persona) => !linkedPersonaIds.has(p.id));
  }, [allPersonas, linkedPersonaIds]);

  const handleLinkPersona = async (personaId: string) => {
    const newPersonaIds = [...userGoal.personaIds, personaId];
    await updateUserGoal.mutateAsync({
      id: userGoal.id,
      updates: { personaIds: newPersonaIds },
    });
  };

  const handleUnlinkPersona = async (personaId: string) => {
    const newPersonaIds = userGoal.personaIds.filter((id) => id !== personaId);
    await updateUserGoal.mutateAsync({
      id: userGoal.id,
      updates: { personaIds: newPersonaIds },
    });
  };

  return (
    <>
      <Card shadow={false}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <FiUser className="text-[var(--primary)]" />
            <h3 className="text-sm font-semibold text-[var(--text)]">Personas</h3>
          </div>
          <Button
            variant="default"
            size="small"
            onClick={() => setIsLinkModalOpen(true)}
          >
            Link
          </Button>
        </div>

        {linkedPersonas.length === 0 ? (
          <p className="text-[var(--text-muted)] text-sm">
            No personas linked to this user goal.
          </p>
        ) : (
          <CardStack layout="stack" gap="sm">
            {linkedPersonas.map((persona: Persona) => (
              <PersonaCard
                key={persona.id}
                persona={persona}
                action={
                  <Button
                    variant="text"
                    size="small"
                    onClick={() => handleUnlinkPersona(persona.id)}
                  >
                    Unlink
                  </Button>
                }
              />
            ))}
          </CardStack>
        )}
      </Card>

      <Modal
        title="Link Persona"
        open={isLinkModalOpen}
        onCancel={() => setIsLinkModalOpen(false)}
        footer={null}
      >
        {availablePersonas.length === 0 ? (
          <p className="text-[var(--text-muted)]">
            No available personas to link. All personas are already linked.
          </p>
        ) : (
          <ul className="space-y-2 max-h-64 overflow-y-auto">
            {availablePersonas.map((persona: Persona) => (
              <li
                key={persona.id}
                className="flex items-center justify-between p-3 rounded border border-[var(--border)] hover:bg-[var(--bg-secondary)]"
              >
                <div>
                  <p className="text-sm font-medium text-[var(--text)]">{persona.name}</p>
                  {persona.role && (
                    <p className="text-xs text-[var(--text-muted)] truncate max-w-xs">
                      {persona.role}
                    </p>
                  )}
                </div>
                <Button
                  variant="primary"
                  size="small"
                  onClick={() => {
                    handleLinkPersona(persona.id);
                  }}
                >
                  Link
                </Button>
              </li>
            ))}
          </ul>
        )}
      </Modal>
    </>
  );
}
