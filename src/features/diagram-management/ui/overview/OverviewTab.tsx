...
import { useRequirementsBySolutionQuery } from '@/entities/requirement';
...
export function OverviewTab({ solutionId, useCaseId }: OverviewTabProps) {
  ...
  const { data: allUseCases = [], isLoading: isLoadingUseCases } = useUseCasesBySolutionQuery(solutionId);
  ...
  const relevantUseCases = useCaseId
    ? allUseCases.filter((uc: UseCase) => uc.id === useCaseId)
    : allUseCases;
