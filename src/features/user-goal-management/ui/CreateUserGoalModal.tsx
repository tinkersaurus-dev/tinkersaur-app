/**
 * CreateUserGoalModal Component
 * Modal for creating new user goals with two tabs:
 * 1. Manual Entry - User enters name and description directly
 * 2. Generate - User provides rough description, LLM generates structured user goal
 */

import { useState, useCallback } from 'react';
import { FiZap } from 'react-icons/fi';
import { Input, Form, useForm, Modal, Button, Tabs } from '@/shared/ui';
import { useCreateUserGoal } from '@/entities/user-goal';
import { useGenerateUserGoal } from '../lib/useGenerateUserGoal';
...
export function CreateUserGoalModal({
  open,
  onClose,
  teamId,
}: CreateUserGoalModalProps) {
  ...
  const { generate, isGenerating, generatedUserGoal, error: generateError, reset: resetGeneration } = useGenerateUserGoal();
  ...
  const handleManualSubmit = async () => {
    ...
    await createUserGoal.mutateAsync({
      teamId,
      name: values.name,
  ...
  const isOkLoading = createUserGoal.isPending;
              ...
              <Form form={manualForm} layout="vertical">
                <div className="space-y-4 pt-4">
                  <Form.Item
                    ...
                    required
                    rules={{
                      required: 'Please enter a user goal name',
          ...
          },
          {
            ...
            children: (
              <div className="space-y-4 pt-4">
                  ...
                  <>
                    <div>
                      ...
                      <Input.TextArea
                        ...
                        placeholder="Enter a rough description of what users need to accomplish. The AI will generate a structured user goal from this."
                        rows={4}
                      />
                    </div>
                ...
                {generateStep === 'preview' && generatedUserGoal && (
                  <>
                    ...
                    <div>
                      ...
                      <Input.TextArea
                        ...
                        placeholder="User goal description"
                        rows={4}
                        maxLength={2000}
                      />
                    </div>
                ...
                {isGenerating && (
                  <div className="flex items-center justify-center py-8">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm text-[var(--text-muted)]">Generating user goal...</p>
                    </div>
                  </div>
                )}
              </div>
