...
export class ApplyPreviewCommand implements Command {
  ...
  async execute(): Promise<void> {
    console.log('[ApplyPreviewCommand] execute() called');
    console.log('[ApplyPreviewCommand] previewShapeId:', this.previewShapeId);

    // Get the preview shape to extract its data
    const previewShape = await this.getShapeFn(this.diagramId, this.previewShapeId);
    console.log('[ApplyPreviewCommand] previewShape:', previewShape);
    ...
    const { id, ...shapeData } = previewShape;
    this.previewShapeData = shapeData;

    // Extract the preview data
    const previewData = previewShape.data as LLMPreviewShapeData;
    console.log('[ApplyPreviewCommand] previewData:', previewData);
    console.log('[ApplyPreviewCommand] previewShapeIds:', previewData?.previewShapeIds?.length);
    console.log('[ApplyPreviewCommand] previewConnectorIds:', previewData?.previewConnectorIds?.length);

    if (!previewData || !previewData.previewShapeIds || !previewData.previewConnectorIds) {
    ...
    console.log('[ApplyPreviewCommand] Updating preview shapes to interactive...');
    const shapeIdMapping = new Map<string, string>();

    // First, collect all shape data
    ...
    // Delete the preview container shape and all preview shapes
    console.log('[ApplyPreviewCommand] Deleting preview container and', oldShapeIds.length, 'preview shapes...');
    ...
    if (this.deleteShapesBatchFn && shapeIdsToDelete.length > 0) {
      ...
      console.log('[ApplyPreviewCommand] Deleted', shapeIdsToDelete.length, 'shapes in batch');
    } else {
    ...
    if (this.addShapesBatchFn && shapeDTOs.length > 0) {
      console.log('[ApplyPreviewCommand] Creating', shapeDTOs.length, 'shapes in batch...');
      ...
      for (let i = 0; i < shapeDTOs.length; i++) {
        ...
        console.log('[ApplyPreviewCommand] Applied shape:', oldShapeIds[i], '->', newShapeId);
      }
      console.log('[ApplyPreviewCommand] Created', shapeDTOs.length, 'shapes in batch');
    } else {
      // Fallback to one-by-one
      console.log('[ApplyPreviewCommand] Creating shapes one by one...');
      for (let i = 0; i < shapeDTOs.length; i++) {
    ...
    // Update connectors to reference the new shape IDs
    // We need to delete and re-create them with the updated shape IDs
    console.log('[ApplyPreviewCommand] Updating connectors to reference new shapes...');

    // First, collect all connector data
    const connectorDTOs: CreateConnectorDTO[] = [];
    const oldConnectorIds: string[] = [];
    ...
    for (const oldConnectorId of previewData.previewConnectorIds) {
      ...
      if (connector) {
        // Get the new shape IDs from the mapping
        const newSourceShapeId = shapeIdMapping.get(connector.sourceShapeId);
        const newTargetShapeId = shapeIdMapping.get(connector.targetShapeId);

        if (!newSourceShapeId || !newTargetShapeId) {
          console.error('[ApplyPreviewCommand] Cannot update connector - shape ID mapping missing:', {
            oldSourceShapeId: connector.sourceShapeId,
            oldTargetShapeId: connector.targetShapeId,
            newSourceShapeId,
            newTargetShapeId,
          });
        ...
        // Prepare connector DTO with updated shape IDs
        ...
        connectorDTOs.push(connectorDTO);
        oldConnectorIds.push(oldConnectorId);
      }
    }

    // Delete all old connectors
    console.log('[ApplyPreviewCommand] Deleting', oldConnectorIds.length, 'preview connectors...');
    if (this.deleteConnectorsBatchFn && oldConnectorIds.length > 0) {
      await this.deleteConnectorsBatchFn(this.diagramId, oldConnectorIds);
      console.log('[ApplyPreviewCommand] Deleted', oldConnectorIds.length, 'connectors in batch');
    } else {
      // Fallback to one-by-one
      for (const oldConnectorId of oldConnectorIds) {
    ...
    if (this.addConnectorsBatchFn && connectorDTOs.length > 0) {
      console.log('[ApplyPreviewCommand] Creating', connectorDTOs.length, 'connectors in batch...');
      ...
      if (diagram) {
        ...
        for (let i = 0; i < connectorDTOs.length; i++) {
          ...
          console.log('[ApplyPreviewCommand] Applied connector:', oldConnectorIds[i], '->', newConnectorId);
        }
        console.log('[ApplyPreviewCommand] Created', connectorDTOs.length, 'connectors in batch');
      }
    } else {
      // Fallback to one-by-one
      console.log('[ApplyPreviewCommand] Creating connectors one by one...');
      for (let i = 0; i < connectorDTOs.length; i++) {
        const diagram = await this.addConnectorFn(this.diagramId, connectorDTOs[i]);
    ...
    console.log('[ApplyPreviewCommand] Apply completed successfully');
  }
