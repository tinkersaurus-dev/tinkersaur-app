    ...
    try {
      ...
      set((state) => {
        ...
        for (const solutionId in newVersions) {
          newVersions[solutionId] = newVersions[solutionId].map((v) => ({
            ...v,
            epics: v.epics.map((e) => (e.id === epicId ? { ...e, ...updated } : e)),
          }));
        }
        return { versions: newVersions, loading: false };
      });
    ...
    try {
      ...
      set((state) => {
        ...
        for (const solutionId in newVersions) {
          newVersions[solutionId] = newVersions[solutionId].map((v) => ({
            ...v,
            epics: v.epics.filter((e) => e.id !== epicId),
          }));
        }
    ...
    try {
      ...
      set((state) => {
        ...
        for (const solutionId in newVersions) {
          newVersions[solutionId] = newVersions[solutionId].map((v) => {
            if (v.id === versionId) {
              const epicMap = new Map(v.epics.map((e) => [e.id, e]));
      ...
      set((state) => {
        ...
        for (const solutionId in newVersions) {
          newVersions[solutionId] = newVersions[solutionId].map((v) => ({
            ...
            epics: v.epics.map((e) => ({
              ...e,
              stories: e.stories.map((s) => (s.id === storyId ? { ...s, ...updated } : s)),
            })),
          }));
        }
        return { versions: newVersions, loading: false };
      ...
      set((state) => {
        ...
        for (const solutionId in newVersions) {
          newVersions[solutionId] = newVersions[solutionId].map((v) => ({
            ...
            epics: v.epics.map((e) => {
              if (e.id === epicId) {
                const storyMap = new Map(e.stories.map((s) => [s.id, s]));
        ...
        for (const solutionId in newVersions) {
          newVersions[solutionId] = newVersions[solutionId].map((v) => ({
            ...
            epics: v.epics.map((e) => ({
              ...
              stories: e.stories.map((s) => ({
                ...s,
                acceptanceCriteria: s.acceptanceCriteria.filter((ac) => ac.id !== id),
              })),
            })),
          }));
        }
