...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    // Get user stories system prompt
    const systemPrompt = getSystemPrompt('user-stories');

    // Prepare the request for the model
    const userMessage = `Generate user stories with acceptance criteria from the following design documentation:\n\n${content}`;
    ...
    const cleanedStories = generatedStories.trim();

    logger.info('Successfully generated user stories');
    return Response.json({
      success: true,
      userStories: cleanedStories,
    });
  } catch (error) {
