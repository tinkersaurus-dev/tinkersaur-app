/**
 * Architecture Toolset Popover Component
 *
 * Displays a popover with architecture tools when the user right-clicks on the canvas.
 * Tools are organized by type (Services, Containers, Utilities) in rows with icon buttons.
 */

import { architectureToolGroups, type Tool } from '../tools';
import type { DrawingConnector } from '~/design-studio/hooks/useInteractionState';
import { ContextMenuWrapper } from '~/design-studio/components/canvas/menus/ContextMenuWrapper';

interface ArchitectureToolsetPopoverProps {
...
export function ArchitectureToolsetPopover({
  x,
  y,
  canvasX,
  canvasY,
  ...
  onToolSelect,
  drawingConnector,
}: ArchitectureToolsetPopoverProps) {
  ...
  return (
    <ContextMenuWrapper
      menuId="architecture-toolset-popover"
      isOpen={isOpen}
      x={x}
      y={y}
      onClose={onClose}
    ...
    >
      ...
      <div className="flex flex-col gap-0">
        {architectureToolGroups.map((group, groupIndex) => (
          <div key={group.type}>
            {/* Tool row */}
            ...
            {groupIndex < architectureToolGroups.length - 1 && (
